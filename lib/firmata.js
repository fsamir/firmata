...
Board.prototype.reportVersion = function (callback) {
    ...
    this.transport.write(new Buffer([REPORT_VERSION]), callback);
};
...
Board.prototype.queryFirmware = function (callback) {
    ...
    this.transport.write(new Buffer([START_SYSEX, QUERY_FIRMWARE, END_SYSEX]), callback);
};
 ...
 * @param {callback} callback A function to call when we serial data has been sent.
 */

Board.prototype.analogWrite = function (pin, value, callback) {
    var data = [];
    ...
    this.transport.write(new Buffer(data), callback);
};
...
Board.prototype.digitalWrite = function (pin, value, callback) {
    ...
    this.transport.write(new Buffer([DIGITAL_MESSAGE | port, portValue & 0x7F, (portValue >> 7) & 0x7F]), callback);
};
